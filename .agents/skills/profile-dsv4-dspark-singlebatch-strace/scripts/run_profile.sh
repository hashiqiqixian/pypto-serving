...
The workload is fixed: one request, DP4/EP16/TP4,
64 prompt tokens, 128 output tokens, DSpark k=7, greedy sampling, ignore_eos=1.
EOF
}
...
if [[ -z ${ARTIFACT_DIR} ]]; then
  ARTIFACT_DIR="${REPO_ROOT}/artifacts/dsv4-dspark-singlebatch-profile-$(date +%Y%m%d-%H%M%S)"
fi
