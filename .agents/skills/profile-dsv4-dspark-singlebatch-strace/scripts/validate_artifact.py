...
# -----------------------------------------------------------------------------------------------------------
"""Validate the fixed DSpark single-batch profile artifact contract."""
...
def main() -> None:
    ...
    for name in ("warmup-responses.json", "responses.json"):
        ...
        if len(responses) != 1 or any(
            int(item.get("prompt_tokens", -1)) != 64
            or int(item.get("completion_tokens", -1)) != 128
            or item.get("finish_reason") != "length"
            for item in responses
        ):
            raise RuntimeError(
                f"{name} does not contain one successful Seq64/Output128 response"
            )
    ...
    decode_roots = [
        event
        for event in host
        if event.get("args", {}).get("callable") == "decode.main+verify"
    ...
    validation = {
        "valid": True,
        "workload": (
            "GBS1/DP4/EP16/TP4/active-DP-group-batch1/Seq64/Output128/"
