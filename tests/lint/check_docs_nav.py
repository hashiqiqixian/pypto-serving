...
# -----------------------------------------------------------------------------------------------------------
"""Fail if mkdocs.yml's nav and the docs/ tree disagree.
...
2. Every page under docs/ is in the nav. A page missing from the nav is reachable
   only by guessing its URL — the site never links to it.
...
def main() -> int:
    ...
    on_disk = {
        ...
        if "_hooks" not in md.parts
    }
    orphans = sorted(on_disk - set(nav_paths))

    if not missing and not orphans:
        print(f"OK: {len(nav_paths)} nav entries cover all {len(on_disk)} pages under docs/")
        return 0
