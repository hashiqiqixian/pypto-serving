...
const ui = {
  ...
  draftAcceptance: document.getElementById("draft-acceptance"),
  acceptanceLength: document.getElementById("acceptance-length"),
  draftTokens: document.getElementById("draft-tokens"),
  speculativeFallback: document.getElementById("speculative-fallback"),
...
async function refreshSummary() {
  try {
    ...
    ui.draftAcceptance.textContent = percent(current.draft_acceptance_rate);
    ui.acceptanceLength.textContent = number(current.mean_acceptance_length, 2);
    ui.draftTokens.textContent = number(current.draft_tokens);
    ui.speculativeFallback.textContent = percent(current.speculative_fallback_rate);
...
function renderDailyTable() {
  if (!state.daily.length) {
    ui.dailyTable.innerHTML = '<tr><td colspan="8" class="empty-cell">No daily data</td></tr>';
    return;
  }
  ui.dailyTable.innerHTML = [...state.daily].reverse().map((row) => {
    ...
    return `<tr><td>${row.day}</td><td>${number(row.request_count)}</td><td>${number(row.prompt_tokens)}</td><td>${number(row.generation_tokens)}</td><td>${hitRate}</td><td>${percent(row.draft_acceptance_rate)}</td><td>${number(row.mean_acceptance_length, 2)}</td><td>${number((row.errors || 0) + (row.aborted || 0))}</td></tr>`;
  }).join("");
}
