...
_SPECULATIVE_COUNTERS = (
    "speculative_drafts", "draft_tokens", "accepted_tokens", "speculative_fallbacks",
)


def _speculative_summary(counters: dict) -> dict:
    drafts = counters.get("speculative_drafts", 0)
    proposed = counters.get("draft_tokens", 0)
    accepted = counters.get("accepted_tokens", 0)
    fallbacks = counters.get("speculative_fallbacks", 0)
    return {
        **{key: counters.get(key, 0) for key in _SPECULATIVE_COUNTERS},
        ...
        "speculative_fallback_rate": fallbacks / (drafts + fallbacks) if drafts + fallbacks else None,
    }
...
class MonitorStore:
        ...
        with self._lock, self._connection:
            ...
            cursor = self._connection.execute(
                ...
                INSERT OR IGNORE INTO samples (
                    timestamp, model_name, running, waiting, kv_cache_usage,
                    request_delta, prompt_token_delta, prefill_token_delta, generation_token_delta,
                    cache_query_delta, cache_hit_delta, preemption_delta, error_delta, aborted_delta,
                    elapsed, ttft_histogram, tpot_histogram,
                    speculative_drafts, draft_tokens, accepted_tokens, speculative_fallbacks
                ) VALUES (
                    ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
                )
                ...
                (
                    ...
                    *(int(counters.get(key, 0)) for key in _SPECULATIVE_COUNTERS),
                ),
            )
            ...
            self._connection.execute(
                ...
                INSERT INTO daily_totals (
                    day, request_count, prompt_tokens, prefill_tokens, generation_tokens,
                    errors, aborted, cache_queries, cache_hits,
                    speculative_drafts, draft_tokens, accepted_tokens, speculative_fallbacks
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(day) DO UPDATE SET
                    ...
                    speculative_drafts = speculative_drafts + excluded.speculative_drafts,
                    draft_tokens = draft_tokens + excluded.draft_tokens,
                    accepted_tokens = accepted_tokens + excluded.accepted_tokens,
                    speculative_fallbacks = speculative_fallbacks + excluded.speculative_fallbacks
                """,
                (
                    day,
                    values["request_delta"],
    ...
    def summary(self, window_seconds: int = 300) -> dict:
        ...
        return {
            ...
            "current": {
                **_speculative_summary({key: sum(row[key] for row in rows) for key in _SPECULATIVE_COUNTERS}),
            ...
            "today": {**today_values, **_speculative_summary(today_values)},
        }
    ...
    def history(self, range_seconds: int) -> list[dict]:
        ...
        with self._lock:
            rows = self._connection.execute(
                ...
                SELECT
                    ...
                    SUM(speculative_drafts) AS speculative_drafts,
                    SUM(draft_tokens) AS draft_tokens,
                    SUM(accepted_tokens) AS accepted_tokens,
                    SUM(speculative_fallbacks) AS speculative_fallbacks
                FROM samples
                WHERE timestamp >= ?
                GROUP BY bucket
                ORDER BY bucket
        ...
        return [{**dict(row), **_speculative_summary(dict(row))} for row in rows]
