...
"""MkDocs build hooks for the PyPTO Serving documentation site.

The docs deliberately link to source files outside ``docs/`` — ``pypto_serving/**``,
``examples/**`` — using repo-relative paths, so a reader browsing the repository on
GitHub can click straight through to the code. MkDocs cannot resolve those targets
(they are not documentation pages), and ``mkdocs build --strict`` turns each one
...
_REPO_URL = "https://github.com/hw-native-sys/pypto-serving"
...
def _repo_url(repo_path: str, was_directory: bool) -> str:
    ...
    GitHub serves directories under ``tree/`` and files under ``blob/``; a ``blob/``
    URL for a directory 404s. A trailing slash in the source link is the signal --
    the alternative, stat-ing the path, would make the build depend on the working
    tree's layout.
    """
...
def _code_spans(markdown: str) -> list[tuple[int, int]]:
    ...
    Matches opening and closing fence types (`` ``` `` vs `` ~~~ ``) and enforces
    that the closing fence is at least as long as the opening fence, per the
    CommonMark spec.
    """
...
def on_page_markdown(markdown: str, page: Any, config: Any, files: Any) -> str:
    ...
    # Directory of this page relative to ``docs/``, e.g. ``user-guide``.
