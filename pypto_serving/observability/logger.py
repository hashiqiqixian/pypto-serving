...
class InMemoryStatLogger(StatLoggerBase):
    ...
    def __init__(
        self, model_name: str, engine_indexes: list[int], *,
        max_model_len: int = 4096, num_speculative_tokens: int = 0,
    ) -> None:
        self.model_name = model_name
        self.server_id = uuid.uuid4().hex
        self.started_at = time.time()
        self._lock = threading.RLock()
        self._vllm_buckets = histogram_buckets(max_model_len)
        self._num_speculative_tokens = num_speculative_tokens
        self._engines: dict[int, _EngineMetrics] = {}
        for index in engine_indexes:
            self._engine(index)
    ...
    def record_queued(self, engine_index: int, request_id: str, *, timestamp: float | None = None) -> None:
        with self._lock:
            state = self._requests.get((engine_index, request_id))
            if state is not None and state.queued_monotonic is None:
                state.queued_monotonic = time.monotonic() if timestamp is None else timestamp

    def record_scheduled(self, engine_index: int, request_id: str, *, timestamp: float | None = None) -> None:
        with self._lock:
            state = self._requests.get((engine_index, request_id))
            if state is not None and state.scheduled_monotonic is None:
                state.scheduled_monotonic = time.monotonic() if timestamp is None else timestamp
        ...
        *,
        ...
        with self._lock:
            ...
            if state.first_token_monotonic is None:
                # vLLM records the complete prompt, including cache hits, on first output.
                engine.processed_prompt_tokens += state.num_prompt_tokens
            ...
            if state.last_token_monotonic is not None and remaining > 0:
                ...
                # vLLM ITL observes one interval per output batch, including speculative batches.
                engine.vllm_histograms["itl"].observe(elapsed)
    ...
    def finish_request(
        ...
        *,
        ...
        with self._lock:
            ...
            histograms = self._engine(engine_index).vllm_histograms
            histograms["max_generation_tokens"].observe(state.num_generation_tokens)
            histograms["n"].observe(1)
            if state.max_tokens is not None:
                histograms["max_tokens"].observe(state.max_tokens)
            scheduled = state.scheduled_monotonic
            queued = state.queued_monotonic
            first = state.first_token_monotonic
            last = state.last_token_monotonic
            if queued is not None and scheduled is not None:
                histograms["queue"].observe(scheduled - queued)
            if scheduled is not None and first is not None:
                histograms["prefill"].observe(first - scheduled)
            if scheduled is not None and last is not None:
                histograms["inference"].observe(last - scheduled)
            if first is not None and last is not None:
                histograms["decode"].observe(last - first)
    ...
    def record_scheduler(self, engine_index: int, stats: SchedulerStats) -> None:
        with self._lock:
            ...
            engine.prefix_cache_query_tokens += stats.prefix_cache_query_tokens
    ...
    def _record_iteration_locked(self, engine_index: int, stats: IterationStats) -> None:
        ...
        engine.speculative_drafts += stats.num_speculative_drafts
        engine.draft_tokens += stats.num_draft_tokens
        engine.accepted_tokens += stats.num_accepted_tokens
        engine.speculative_fallbacks += stats.num_speculative_fallbacks
        positions = stats.num_accepted_tokens_per_pos
        if len(positions) > len(engine.accepted_tokens_per_pos):
            engine.accepted_tokens_per_pos.extend([0] * (len(positions) - len(engine.accepted_tokens_per_pos)))
        for pos, count in enumerate(positions):
            engine.accepted_tokens_per_pos[pos] += count
        if stats.num_iteration_tokens is not None:
            engine.vllm_histograms["iteration_tokens"].observe(stats.num_iteration_tokens)
        ...
        for finished in stats.finished_requests:
            ...
            engine.vllm_histograms["e2e"].observe(finished.e2e_latency)
            engine.vllm_histograms["prompt_tokens"].observe(finished.num_prompt_tokens)
            engine.vllm_histograms["generation_tokens"].observe(finished.num_generation_tokens)
            engine.vllm_histograms["tpot"].observe(finished.mean_time_per_output_token or 0.0)
    ...
    def _engine(self, engine_index: int) -> _EngineMetrics:
        if engine_index not in self._engines:
            self._engines[engine_index] = _EngineMetrics(
                accepted_tokens_per_pos=[0] * self._num_speculative_tokens,
                vllm_histograms={key: _Histogram(buckets) for key, buckets in self._vllm_buckets.items()},
            )
    ...
    def _engine_snapshot(index: int, metrics: _EngineMetrics) -> dict:
        return {
            ...
            "gauges": {
                ...
                "draft_acceptance_rate": (
                    metrics.accepted_tokens / metrics.draft_tokens if metrics.draft_tokens else None
                ),
                "mean_acceptance_length": (
                    1 + metrics.accepted_tokens / metrics.speculative_drafts
                    if metrics.speculative_drafts else None
                ),
            },
            "counters": {
                "prompt_tokens": metrics.prompt_tokens,
                "processed_prompt_tokens": metrics.processed_prompt_tokens,
                ...
                "prefix_cache_query_tokens": metrics.prefix_cache_query_tokens,
                "prefix_cache_hits": metrics.prefix_cache_hits,
                "preemptions": metrics.preemptions,
                "speculative_drafts": metrics.speculative_drafts,
            ...
            "accepted_tokens_per_pos": list(metrics.accepted_tokens_per_pos),
            "vllm_histograms": {key: value.snapshot() for key, value in metrics.vllm_histograms.items()},
            "histograms": {
