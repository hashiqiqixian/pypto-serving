...
# -----------------------------------------------------------------------------------------------------------
"""Checkpoint storage contracts for the V4.1 text backbone, including Engram.
...
Vision and DSpark tensors are outside this function's scope. The stored VL router
bias is included so it is recognized, although text-only forward does not use it.
"""
...
def backbone_weight_specs(config: Mapping[str, Any]) -> dict[str, TensorSpec]:
    ...
    engram_ids, engram_rows = text.get("engram_layer_ids", ()), text.get("engram_num_embeddings", ())
    if len(ratios) < layers or len(engram_ids) != len(engram_rows):
        raise ValueError("compression or Engram layer metadata has inconsistent lengths")
    if len(set(engram_ids)) != len(engram_ids):
        raise ValueError("engram_layer_ids contains duplicates")
    if any(type(i) is not int or not 0 <= i < layers for i in (*kv_sources, *index_sources, *engram_ids)):
    ...
    add("head.weight", (vocab, dim), "BF16", "tp_shard_axis0;cast_bf16_to_fp32")
    ...
    for layer in range(layers):
        ...
        add(base + ".ffn.gate.bias_vl", (experts,), "F32", "retain_unused_text_bias")
        ...
        if layer in engram_ids:
            rows = engram_rows[engram_ids.index(layer)]
            embed_dim = positive("engram_head_dim")
            if embed_dim % 32:
                raise ValueError("engram_head_dim must be divisible by 32")
            hash_cols = (positive("engram_max_ngram_size") - 1) * positive("engram_n_heads")
            prefix = base + ".engram"
            add(
                prefix + ".embed.weight",
                (rows, embed_dim),
                "F8_E4M3",
                "row_shard_ceil_pad_zero;dequantize_selected_rows_on_lookup",
            )
            add(
                prefix + ".embed.scale",
                (rows, embed_dim // 32),
                "F8_E8M0",
                "row_shard_ceil_pad_one;preserve_ue8m0_per_row_block32",
            )
            add(prefix + ".q_weight", (hc, dim), "BF16")
            add(prefix + ".k_weight", (hc, dim), "BF16")
            dense(prefix + ".wkv", dim * (hc + 1), hash_cols * embed_dim)
    return specs
